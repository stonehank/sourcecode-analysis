...
整体：
* 调用了回调函数 cancel和ok
* 保存了点击打开dialog的位置，用作展开dialog动画的初始点
* 配置了footer上按钮的语言，默认为英文

* 最后调用[rc-dialog](../rc-dialog/DialogWrap.md)
...
export default class Modal extends React.Component<ModalProps, {}> {
  ...
  componentDidMount() {
    ...
    addEventListener(document.documentElement, 'click', (e: MouseEvent) => {
      ...
      // 否则直接 zoom 展示
      // 这样可以兼容非点击方式展开而不会导致展开点错乱
      setTimeout(() => mousePosition = null, 100);
    });
    mousePositionEventBinded = true;
  }

  // 此处参数就是 defaultLocale
  // 设置语言用的
  // { okText: 'OK',
  //   cancelText: 'Cancel',
  //   justOkText: 'OK',
  //  }
  // （在 LocaleReceiver 组件里转换）
